...
cat << _EOF_ | $SUDO tee -a /etc/puppetlabs/puppet/puppet.conf
  reports = puppetdb
  storeconfigs = true
...
cat << _EOF_ | $SUDO tee /etc/puppetlabs/puppet/routes.yaml
---
...
cat << _EOF_ | $SUDO tee /etc/puppetlabs/puppetdb/conf.d/jetty.ini
[jetty]
